...
class TestSrcRec(unittest.TestCase):
    ...
    def test_average_conflicting_receivers_uses_count_weighted_mean(self):
        sr = SrcRec("unused")
        sr.src_points = pd.DataFrame({
            "event_id": ["EVENT_0", "EVENT_1"],
            "evla": [1.0, 2.0],
        ...
        sr.update_unique_src_rec(conflicting_receiver_action="average")

        # (10, 30, 100) occurs 3 times and (20, 40, 200) once
        expected = np.array([12.5, 32.5, 125.0])
        self.assertEqual(sr.rec_points.shape[0], 2)
        self.assertTrue(
            np.allclose(
                sr.rec_points[["stla", "stlo", "stel"]].to_numpy(dtype=float),
        ...
        self.assertTrue(
            np.allclose(
                sr.rec_points_cs.loc[0, ["stla2", "stlo2", "stel2"]]
                .to_numpy(dtype=float),
                np.zeros(3),
            )
        )
